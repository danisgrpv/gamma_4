...
def get_coeff_simpson_nonuniform(responce, grid, EDGE, min_index=None, max_index=None):
    simpson_coeff_list = []
    new_energy = []
    ...
    def polinom_coeff_c0(i):
        c0 = (6*(grid[i+2]-grid[i])*(grid[i+1]-grid[i]) + 2*(grid[i+2]-grid[i])**2 - 3*(grid[i+2]-grid[i])*(grid[i+1]-grid[i]+grid[i+2]-grid[i])) / (6*((grid[i+1]-grid[i])))
        return c0

    def polinom_coeff_c1(i):
        c1 = ((grid[i+1]-grid[i-1]))**3 / (6*(grid[i]-grid[i-1]) * (grid[i+1]-grid[i]))
        return c1

    def polinom_coeff_c2(i):
        c2 = ((grid[i]-grid[i-2]) * (2*(grid[i]-grid[i-2]) - 3*(grid[i-1]-grid[i-2]))) / 6*((grid[i]-grid[i-1]))
        return c2
        ...
        if i not in RIGHT_EDGE:
            ...
            if i % 2 != 0 and i != min_index and i != max_index - 1:
                ...
                if reverse == False:
                    # если i-скачок
                    if i in EDGE:
                        simpson = (1/2)*(grid[i]-grid[i-1])*responce[i] + polinom_coeff_c0(i+1)*responce[i+1] # К-нечетное-скачок
                        write()
                        continue
                    # если i без скачка
                    if i not in EDGE:
                        simpson = polinom_coeff_c1(i)*responce[i] # К-нечетное
                ...
                if reverse == True:
                    # если i-скачок
                    if i in EDGE:
                        simpson = polinom_coeff_c2(i)*responce[i] + polinom_coeff_c0(i+1)*responce[i+1] # К-четное-скачок
                        write()
                        reverse = False
                        continue
                    # если i без скачка
                    if i not in EDGE:
                        simpson = polinom_coeff_c2(i)*responce[i] + polinom_coeff_c0(i)*responce[i] # К-четное
                        write()
