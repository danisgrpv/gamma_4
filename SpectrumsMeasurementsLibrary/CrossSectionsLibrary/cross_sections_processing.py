...
def edge_to_dict(number, marker, dataframe):
    ...
    :param marker: флаг, имеет два значения
        ...
        если marker_side='energy' - возвращаются значения энергий К-скачков
    Возвращаемые значения: списки со значениями энергий и сечений K-скачов
    """
...
def get_edges_list(number, marker, initial_meshs, initial_cross_section):
    """
    ...
    Функции необходимо передать массив энергетическую сетку и массив всех исходных сечений
    """

    # распаковка списка materials_list
    energy = initial_meshs[number]
    cross_section = initial_cross_section[number]
...
def interpolate_with_edge_from_minimal_en(dataframe, general_grid, min_en):
    """
    Функция добавляет скачки в данных с необработанными скачками. В отличие от функции interpolate_with_edge
    может обрабатывать данные, полученные на раличных сетках. Передача параметра min_en позволяет избежать ошибки
    при поиске скачков с энергией, которой нет на используемой сетке
    """
    materials = dataframe_to_list(dataframe)
    gg = general_grid
